...
- [ ] Create `src/components/AccordionItem.tsx` — a single collapsible FAQ row. Props: `question: string`, `answer: string`, `isOpen: boolean`, `onToggle: () => void`. Animated height transition using Tailwind's `transition-all duration-300`. Chevron icon rotates 180° when open. Full keyboard support (Enter/Space to toggle, proper `aria-expanded` and `aria-controls`).
- [ ] Create `src/components/Accordion.tsx` — wraps a list of `AccordionItem` components. Manages open/close state (only one item open at a time). Accepts `items: FaqItem[]`.

### Projects Page (`src/pages/Projects.tsx`)

- [ ] Create `src/pages/Projects.tsx` with `<Helmet>`:
...
- [ ] **Page Hero:** Same banner pattern as Services:
  - `<h1>` "Våra projekt"
...
- [ ] **Call-to-Action:** Below the grid — "Vill du se mer? Kontakta oss för en kostnadsfri konsultation." with a "Begär offert" button

### About Page (`src/pages/About.tsx`)

- [ ] Create `src/pages/About.tsx` with `<Helmet>`:
...
- [ ] **Company Story Section:** Two-column layout on desktop (text left, placeholder image right):
  - `<h2>` "Vår historia"
...
- [ ] **Safety Philosophy Section:** Blue background with white text:
...
- [ ] Create `src/pages/Contact.tsx` with `<Helmet>`:
...
- [ ] **Contact Info + Map Section:** Two-column layout on desktop:
...
- [ ] **Quick Contact CTA:** Bottom banner — "Snabbaste sättet att nå oss är via telefon eller e-post. Vi svarar normalt inom några timmar på vardagar." with click-to-call and click-to-email buttons side by side.

- [ ] Run `npm run dev` and verify all three pages render correctly at mobile and desktop breakpoints

...
- [ ] Create `src/data/contact.ts` with the company's contact details:
  ```ts
...
- [ ] Create `src/data/services.ts` with all five service types as an array of `ServiceItem`:
  ```ts
...
- [ ] Create `src/data/projects.ts` with placeholder project entries:
  ```ts
...
- [ ] Create `src/data/faq.ts` with common scaffolding questions:
  ```ts
...
- [ ] Create `src/data/index.ts` that re-exports everything for clean imports:
  ```ts
  export * from './contact'
...
- [ ] Verify TypeScript compiles cleanly with no errors on the data files: `npx tsc --noEmit`

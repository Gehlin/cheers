...
- [x] Update `tailwind.config.js` to extend the theme with brand tokens:
  ```js
  ...
  ```
- [x] Update `src/index.css` to import Tailwind directives and set global base styles:
  ```css
  @tailwind base;
  @tailwind components;
  @tailwind utilities;
  ...
  ```
- [x] Create `src/components/Header.tsx` — sticky header with:
...
- [x] Create `src/components/Footer.tsx` with three columns:
...
- [x] Create `src/components/Button.tsx` — reusable button with variants: `primary` (amber fill), `secondary` (blue fill), `outline` (bordered). Accepts `as="a"` or `as="button"`, `href`, `onClick`, `disabled`, `loading` (shows spinner). Full keyboard focus styles.
- [x] Create `src/components/SectionHeading.tsx` — reusable `<h2>` component with optional subtitle `<p>`, centered or left-aligned variant, and a decorative amber underline/bar.
- [x] Create `src/pages/NotFound.tsx` — Swedish 404 page:
...
- [x] Create `src/App.tsx` — set up `BrowserRouter`, `Routes` with placeholders for all routes (Hem `/`, Tjänster `/tjanster`, Projekt `/projekt`, Om oss `/om-oss`, Kontakt `/kontakt`, Begär offert `/begar-offert`, 404 catch-all `*`). Wrap routes with `<Header>` and `<Footer>`. Add `id="main-content"` to the `<main>` element wrapping `<Outlet>` or route children.
- [x] Wrap the app in `<HelmetProvider>` from `react-helmet-async` in `src/main.tsx`
- [x] Create `src/types/index.ts` with shared TypeScript interfaces (to be expanded in later phases):
  ```ts
  ...
  ```
- [x] Verify layout renders correctly with placeholder page content at all routes: `npm run dev`
  - Build passed: `tsc -b && vite build` — 38 modules, 0 errors. CSS 12.47 kB, JS 258 kB.
